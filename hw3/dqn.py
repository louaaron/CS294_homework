...
def learn(env,
    ...
    y = rew_t_ph + gamma * tf.multiply((1.0 - done_mask_ph), max_q)
    q_t_taken = tf.reduce_sum(tf.multiply(q_t, tf.one_hot(act_t_ph, num_actions)), axis = 1)

    total_error = tf.losses.mean_squared_error(y, q_t_taken)
